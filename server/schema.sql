...
-- Redis is the active distributed coordination layer; these server-only tables
-- preserve permissions and the latest CRDT snapshot across complete outages.
...
CREATE INDEX IF NOT EXISTS collaboration_rooms_project_idx ON public.collaboration_rooms(project_id);
CREATE INDEX IF NOT EXISTS collaboration_members_user_idx ON public.collaboration_members(user_id, active);
