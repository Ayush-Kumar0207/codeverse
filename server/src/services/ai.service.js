...
async function generateNeuralInsight(payload) {
  ...
  for (const modelName of prepared.modelsToTry) {
    try {
      ...
      if (suggestion) {
        return { suggestion, model: modelName };
      }
  ...
  throw new HttpError(
    502,
    `AI assistant is unavailable. Tried ${prepared.modelsToTry.join(", ")}.`,
    failures.join(" | ")
  );
}

function prepareGeneration(payload) {
  const { prompt, context, systemPrompt, model, maxTokens, fast } = payload;
...
`;
  ...
  return {
    ...
    fullPrompt,
    localReply,
    modelsToTry: uniqueModels([model, ...FALLBACK_MODELS]),
  };
}
    ...
    try {
      ...
      for await (const chunk of ollamaRes.data) {
        ...
        while (newlineIndex >= 0) {
          ...
          if (item.done) return { model: modelName };
        }
      }
      ...
      if (emitted) return { model: modelName };
