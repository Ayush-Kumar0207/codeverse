...
| Server tests | `npm run test:server` | 30 API, security, runtime, multilingual AST/compiler, EvidenceOS, deployment, and live Socket.IO tests; the production Docker proof is opt-in locally and mandatory in its CI job |
...
GitHub Actions runs every layer for pull requests and pushes to `main`, requires all six native language toolchains, and separately executes the digest-pinned production EvidenceOS Docker path. Failed browser runs retain Playwright traces, screenshots, and videos for diagnosis. Dependabot proposes grouped dependency updates so maintenance remains reviewable.
