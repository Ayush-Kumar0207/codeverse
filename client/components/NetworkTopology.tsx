...
type SyncPhase = "idle" | "uploading" | "verifying" | "complete" | "failed";
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        !hasError && !isSyncing && isSynced && "border-emerald-400/20 bg-emerald-400/10 text-emerald-300",
        !hasError && !isSyncing && !isSynced && "border-white/10 bg-white/[0.04] text-white/40"
      )}>
        {stateLabel}
      </div>
      ...
      <div className="absolute inset-x-0 top-[4.7rem] flex items-center justify-between px-7 sm:px-14">
        ...
        <NodeIcon icon={Cloud} label="Cloud API" state={hasError ? "error" : isSyncing || isSynced ? "active" : "idle"} pulse={isSyncing && phase === "uploading"} />
        <NodeIcon icon={Database} label="Snapshot history" state={isSyncing && phase === "verifying" || isSynced ? "active" : "idle"} pulse={isSyncing && phase === "verifying"} />
      </div>
    </div>
  );
}
