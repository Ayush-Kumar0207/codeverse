...
import { createAlgorithmWorkspace } from "@/features/workspace/algorithm-demo-workspace";
...
describe("demo workspace factory", () => {
  ...
  it("builds an algorithm workspace with a runnable solution and problem brief", () => {
    ...
    const workspace = createAlgorithmWorkspace({ algoId: algorithm!.id, preferredLanguage: "C++" });
    expect(workspace).not.toBeNull();
  ...
  it("selects the cinematic trace when 3D mode is requested", () => {
    ...
    const workspace = createAlgorithmWorkspace({ algoId: algorithm.id, visualizerMode: "3d" });
    expect(workspace).not.toBeNull();
